...
extension JSValue {
    static public func data<T: Encodable>(_ value: T) throws -> Data {
        throw "nyi"
    }
}

protocol GenerateEncodable {}
...
extension LanguageServerResponse: Encodable {
    ...
    private func encode(to encoder: Encoder, id: RequestId, encodables: [Encodable]?) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encode("2.0", forKey: .jsonrpc)
        try container.encode(id, forKey: .id)
        if let encodables = encodables { try container.encode(encodables, forKey: .result) }
    }

    private func encode(to encoder: Encoder, id: RequestId, encodable: Encodable?) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encode("2.0", forKey: .jsonrpc)
        try container.encode(id, forKey: .id)
        if let encodable = encodable { try container.encode(encodable, forKey: .result) }
    }

    private func encode(to encoder: Encoder, id: RequestId) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encode("2.0", forKey: .jsonrpc)
        try container.encode(id, forKey: .id)
        try container.encode([String:String](), forKey: .result)
    }

    public func encode(to encoder: Encoder) throws {
        switch self {
        case let .initialize(requestId, result): try encode(to: encoder, id: requestId, encodable: result)
